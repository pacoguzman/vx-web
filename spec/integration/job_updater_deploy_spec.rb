...
describe JobUpdater, "(deploy jobs)", type: "model" do
  ...
  context "only deploy tasks" do
    it "when all jobs successfuly completed" do
      ...
      job1  = create :job, build: build, number: 1, status: 0, kind: 'deploy'
      job2  = create :job, build: build, number: 2, status: 0, kind: 'deploy'
  ...
  context "regular and deploy tasks" do
    ...
    it "when all jobs successfuly completed" do
      build = create :build, status: 0
      job1  = create :job, build: build, number: 1, status: 0, kind: 'regular'
      job2  = create :job, build: build, number: 2, status: 0, kind: 'deploy'
