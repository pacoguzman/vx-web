...
describe CompanyUsage do
  describe '#calculate' do
    ...
    it 'returns current company_usage' do
      ...
      today_job = create(:job, build: today_build, status: "passed", started_at: now - 20.minutes, finished_at: now - 10.minutes - 20.seconds)
      today_job.create_job_history!

      yesterday_build = create(:build, project: project, number: 2)
      yesterday_job = create(:job, build: yesterday_build, status: "passed", started_at: (now - 1.day).beginning_of_day + 1.hour, finished_at: (now - 1.day).beginning_of_day + 2.hour)
      yesterday_job.create_job_history!

      month_build = create(:build, project: project, number: 1)
      month_job = create(:job, build: month_build, status: "passed", started_at: now - 20.days, finished_at: now - 20.days + 4.hours)
